...
class Tournament:
    def __init__(self, population=None, pop_size=4, initial_game_length=10, game_length_step=10,
                 max_game_length=1000, num_gen=100, training_ai=BasicAI.RandomAI()):
    ...
    def train(self):
        for epoch in range(self.num_gen):
            ...
            # Pick any random AI
            second0 = copy.deepcopy(self.population[random.randint(1, len(self.population)-1)])
            ...
            pickle.dump(first0, open("temp/best_nn_gen.p", "wb"))

            # self.population = [first0, first1, first2, second0, second1, second2, third, new_agent]
            self.population = [first0, first1, first2, second0]
