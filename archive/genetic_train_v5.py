...
class Tournament:
    def __init__(self, population=None, pop_size=50, initial_game_length=20, game_length_step=5,
        ...
        if self.population is None:
            ...
            for i in range(pop_size):
                self.population.append(Neural_Genetic.NN2())
    ...
    def train(self):
        for epoch in range(self.num_gen):
            ...
            pickle.dump(self.population[0], open("temp2/best_nn_gen_epoch" + str(epoch) + ".p", "wb"))
...
def train_algorithm():
    ...
    for i in range(8):
        ...
        tourneys[i].train()
        pickle.dump(tourneys[i].get_best(), open("ai3/best_nn_gen_" + str(i) + ".p", "wb"))

    final_pop = [t.get_best() for t in tourneys]

    last_tourney = Tournament(population=final_pop)
    pickle.dump(last_tourney.get_best(), open("ai3/best_nn.p", "wb"))
